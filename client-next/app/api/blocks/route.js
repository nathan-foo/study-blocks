...
import { quizOutlineFromPdf } from "@/models/quiz-pdf";
import { flashcardOutlineFromPdf } from "@/models/flashcards-pdf";

export async function POST(request) {
    const { blockId, createdBy, topic, difficulty, url } = await request.json();
    ...
    if (!url) {
        ...
        const courseResponse = await courseOutlineFromPdf(url);
        const outline = JSON.parse(courseResponse.response.text());

        await connectDB();
        await Block.create({ blockId, createdBy, topic, difficulty, outline: outline, url });
        return NextResponse.json({ message: "Block Created" }, { status: 201 });
    }
}
...
export async function PATCH(request) {
    const { topic, difficulty, url } = await request.json();
    ...
    if (!url) {
        ...
        const quizResponse = await quizOutlineFromPdf(url);
        const quiz = JSON.parse(quizResponse.response.text());

        const flashcardResponse = await flashcardOutlineFromPdf(url);
        const flashcards = JSON.parse(flashcardResponse.response.text());
